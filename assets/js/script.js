...
function changeLang(val)
{
	...
	dataString.append('method', 'saveLang');
	dataString.append('lang', val);
	$.ajax({
		dataType: 'json',
		type: "POST",
...
{
	var dataString = new FormData();
	dataString.append('method', 'loadProductDetail');
	...
	$.ajax({
		...
		success: function (responseData) 
		{
			if(responseData.RESULT==1)
			{
				$("#email-verify-form")[0].reset()
				$('#VerifyOtp').val('');
				$('#askEmail').modal('show');
				$('.SendEmailOtpDiv').show();
				$('.VerifyOtpFormDiv').hide();
				$('#emailVerifyProductId').val(id);
			}
			else if(responseData.RESULT==2)
			{
				if(responseData.FileType=='image')
				{
					$('.ShowImageDiv').attr('src',responseData.file);
					$('#ShowImage').modal('show');
				}
				else
				{
					window.open(responseData.file, "_blank");
				}
			}
			else 
			{
...
if($('#email-verify-form').length){
	$('#email-verify-form').validate({
		submitHandler: function (form)
		{
			$("#email-verify-submit").attr("disabled", true);
			var dataString = new FormData(form);
			dataString.append('method', 'sendEmailOTP');
			$.ajax({
			    dataType: 'json',
                type: "POST",
				url: "scripts/ajax/index.php",
                ...
                success: function (responseData) 
				{
					$("#email-verify-submit").attr("disabled", false);
					if(responseData.RESULT==1)
					{
						$('.SendEmailOtpDiv').hide();
						$('.VerifyOtpFormDiv').show();
						$('#otpVerifyID').val(responseData.OTPID);
						$('#VerifyOtp').val('');
					}
					else
					{
...
if($('#VerifyOtpForm').length){
	$('#VerifyOtpForm').validate({
		submitHandler: function (form)
		{
			...
			var dataString = new FormData(form);
			dataString.append('method', 'sendEmailOTPverify');
			$.ajax({
			    dataType: 'json',
                type: "POST",
				url: "scripts/ajax/index.php",
		...
		submitHandler: function (form)
		{
			...
			$.ajax({
                ...
                success: function (responseData) 
				{
					...
					{
						$('.contact-form-msg').html('<p class="blueText">Thank you, our team will contact you soon.</p>');
						$('#contact-form')[0].reset();
					}
					else
					{
						$('.contact-form-msg').html('<p class="redText">Try Again.</p>');	
					}
                },
                error: function (responseData) {
                    console.log('Ajax request not recieved!');
